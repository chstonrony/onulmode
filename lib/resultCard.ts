...
const EXTRA_POOL = [
  "눅눅함", "마음침수", "속끓임", "버텨짐", "새벽됨",
  "텅울림", "혼자삭힘", "멀쩡한척", "생각폭주", "울컥전",
  "감정증발", "뇌과부하", "비어버림", "간신유지", "감정곰팡이",
  "억지로괜찮음", "흐물거림", "멍밤",
] as const;
...
export function generateResult(emotions: string[], seed?: number): ResultData {
  ...
  return {
    ...
    adminWarning:    pick(ADMIN_WARNINGS_POOL,  rng),
    checkboxItems:   [...CHECKBOX_POOL].sort(() => rng() - 0.5).slice(0, 3),
