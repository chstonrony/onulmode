      ...
      <AnimatePresence>
        {flyingCards.map(card => (
          <motion.div
            ...
            animate={{
              // 단계:  시작  →  커짐  →  유지  →  이동   →  도착   →  찌그러짐
              x:       [0,     0,      0,      card.dx * 0.88, card.dx],
              y:       [0,     0,      0,      card.dy * 0.88, card.dy],
              scale:   [1,     1.28,   1.28,   0.72,           0.22],
              rotate:  [card.initRot, card.initRot + 4, card.initRot + 4, 28, 35],
              opacity: [1,     1,      1,      0.9,            0],
              scaleX:  [1,     1,      1,      1,              0.4],
            }}
            transition={{
              duration: 1.3,
              times:    [0, 0.1, 0.22, 0.88, 1],
              ease:     [0.22, 1, 0.36, 1],
            }}
